...
type complexe = F of float | C of float * float;;

(*on souhaite utiliser des liste d'ope ET d'entiers à la fois*)
type ope_int = L1 of int | Point | I | L2 of ope | L3 of parenthese | L4 of ope_int list | L5 of float | L6 of complexe;;


let simplifie c = match c with
  | F(i) -> F(i)
  | C(i1,i2) -> 
    if i2 = 0. then F(i1)
    else c;;
...
let conjugue z = match z with
  | F(i) -> z
  | C(i1,i2) -> C(i1,-.i2);;


let (++) z1 z2 = match (z1,z2) with
  | (F(i1),F(i2)) -> F(i1 +. i2)
  | (F(i1),C(i2,i3)) -> simplifie (C(i1 +. i2, i3))
  | (C(i1,i2),F(i3)) -> simplifie (C(i1 +. i3, i2))
  | (C(i1,i2),C(i3,i4)) -> simplifie (C(i1 +. i3, i2 +. i4));;
...
let ($) z1 z2 = match (z1,z2) with
  | (F(i1),F(i2)) -> F(i1 *. i2)
  | (F(i1),C(i2,i3)) -> simplifie (C(i1 *. i2, i1 *. i3))
  | (C(i1,i2),F(i3)) -> simplifie (C(i1 *. i3, i2 *. i3))
  | (C(i1,i2),C(i3,i4)) -> simplifie (C(i1 *. i3 -. i2 *. i4, i1 *. i4 +. i2 *. i3));;


let (//) z1 z2 = match z2 with
  | F(i)-> z1 $ F(1./.i)
  | C(i1,i2) -> z1 $ (conjugue z2) $ F(1./.(i1 ** 2. +. i2 ** 2.));;


let ($$) z1 z2 = match (z1,z2) with (*la puissance z2, z2 doit être un entier*)
  | (F(i1),F(i2)) -> F(i1**i2)
  | (F(i1),C(i2,i3)) -> 
    if i3 <> 0. then failwith "Seule la puissance d'un nombre complexe avec exposant entier, et la puissance d'un nombre réelle avec exposant réelle sont définies ici"
    else F(i1 ** i3) (*ocaml renverra tout seul une erreur si i1 < 0*)
  | (C(i1,i2),F(i3)) -> 
    if i2 = 0. then F(i1 ** i3) (*ocaml renverra tout seul une erreur si i1 < 0*)
        else 
          if floor i3 <> i3 || i3 < 0. then failwith "Seule la puissance d'un nombre complexe avec exposant entier, et la puissance d'un nombre réelle avec exposant réelle sont définies ici"
          else
            let rec aux z n0 =
              if n0 = 0. then F(1.)
              else z $ (aux z (n0-.1.)) in
              aux z1 i3
  | (C(i1,i2),C(i3,i4)) ->
    if i4 <> 0. then failwith "Seule la puissance d'un nombre complexe avec exposant entier, et la puissance d'un nombre réelle avec exposant réelle sont définies ici"
    else
      if i2 = 0. then F(i1 ** i3) (*ocaml renverra tout seul une erreur si i1 < 0*)
      else 
        if floor i3 <> i3 || i3 < 0. then failwith "Seule la puissance d'un nombre complexe avec exposant entier, et la puissance d'un nombre réelle avec exposant réelle sont définies ici"
...
let rec reconnait_cplx l = match l with
  | [] -> []
  | L5(p) :: q -> 
    begin
    match q with
      | I :: q1 -> L6(C(0.,p)) :: (reconnait_cplx q1)
      | _ -> L6(F(p)) :: (reconnait_cplx q)
    end
  | L4(l) :: q -> L4(reconnait_cplx l) :: (reconnait_cplx q)
  | L2(e) :: q -> L2(e) :: (reconnait_cplx q)
  | L3(p) :: q -> L3(p) :: (reconnait_cplx q);;
...
let rec parentheses l = 
  ...
  let rec aux l1 lf compteur = match l1 with
    ...
    match l with
      | [] -> []
      | L6(z) :: L3(p) :: q -> L6(z) :: L2(Mult) :: (parentheses (L3(p) :: q)) (*on ajoute les multiplications sous entendues*)
      | L3(p) :: q ->
        if p = Ouverte then 
...
let list_of_str s =
  ...
  let rec aux_int i l c = 
    ...
    let rec aux i l =
      ...
      parentheses (reconnait_cplx (reconnait_float (aux (n-1) [])));;
...
let calcul s =
  let l = list_of_str s in 
    match calc (List.rev l) with
      | F(i) -> string_of_float i
      | C(i1,i2) -> string_of_float i1 ^ " + i * " ^ string_of_float i2;;

let l = "(289*5i/7)+495/25";;
list_of_str l;;
calcul l;;
495./.25.;;